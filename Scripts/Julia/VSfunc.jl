...
function VSfigure(data, figs, type, clickfreq, layer)

    foldername = "VectorStrength"
    ...
    savename = FileName[1:end-4] * "_" * layer * ".png"
    savefig(vstrengthplot, joinpath(figs,"VectorStrength",savename))

    #rule out anything non-significant
    CurRun = CurRun[CurRun[:,:P] .< 0.001,:]

    vstrengthplot = @df CurRun groupedboxplot(:Measurement,:VectorStrenght, group = :Group, markerstrokewidth=0, ylims = (0,1), legend = false) 
    savename = "Corrected" * FileName[1:end-4] * "_" * layer * ".png"
    savefig(vstrengthplot, joinpath(figs,"VectorStrength",savename))

    mphaseplot = @df CurRun groupedviolin(:Measurement,:MeanPhase, group = :Group, ylims = (0,6.5), legend = false) #BIMODAL!!!
    savename = "Phase" * FileName[1:end-4] * "_" * layer * ".png"
    savefig(mphaseplot, joinpath(figs,"VectorStrength",savename))
